-- This is an updated version of LeanDojo v2's ExtractData.lean that works
-- in this project.
...
private def visitTermInfo (ti : TermInfo) (env : Environment) : TraceM Unit := do
  ...
  -- Lean 4.11.0: findLean panics for core library modules whose source
  -- is not present under .lake/packages/lean4/src/. Skip such premises.
  let p ← Path.findLean modName
  let mut defPath := toString p
...
def shouldProcess (path : FilePath) (noDeps : Bool) : IO Bool := do
  ...
  -- NEW VERSION
  let some oleanPathOld := Path.toBuildDir "lib/lean" relativePath "olean" |
    throw $ IO.userError s!"Invalid path: {path}"
  let some oleanPathNew := Path.toBuildDir "lib" relativePath "olean" |
    throw $ IO.userError s!"Invalid path: {path}"
  return (← oleanPathOld.pathExists) || (← oleanPathNew.pathExists)
